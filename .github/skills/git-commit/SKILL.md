...
style(resource): Checkstyle 指摘のインデント・空白を修正
docs(repository): ExampleRepository に Javadoc を追加
```
...
## 8. マイグレーションワークフロー生成ファイル（コミット禁止）

以下のファイルは **マイグレーション作業中にローカルで自動生成される作業用ファイル** であり、リポジトリにコミットしてはならない

| ファイル | 用途 | 禁止理由 |
|---|---|---|
| `sonar-project.properties` | sonar-scanner-cli の解析オプション設定 | 本リポジトリ固有の SonarQube 環境情報を含むため |
| `lombok.config` | JaCoCo カバレッジ測定時の Lombok 生成コード除外設定 | 解析目的の局所設定でありプロダクトコードではないため |

### `pom.xml` への JaCoCo 追加について

`pom.xml` への JaCoCo 追加を行った場合、コミットしないこと
