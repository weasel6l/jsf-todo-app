...
- 全てのテキストファイルは改行（newline）で終わること
  ...
  - PowerShell で `[System.IO.File]::WriteAllText` を使う場合、**文字列末尾に必ず改行を付けること**

> **警告（PowerShell here-string）**: `@"..."@` 形式を使い、最後の `}` の後に
> 必ず空行（= LF 1 文字）を追加すること
> `@'...'@` 形式は本プロジェクトでは **使用禁止**（「PowerShell ファイル作成の注意」参照）
