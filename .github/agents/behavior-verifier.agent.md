...
### 3-5. 移行元 JSF の到達不能コード検出

JSF Backing Bean（`jsf_backing_beans` Memory および実際の `.java` ファイル）を読み込み、到達不能なコードパスが存在するかを確認する。

**到達不能コードの典型パターン**:

| パターン | 例 |
|---|---|
| `return` / `throw` の直後に実行可能居がある | `return result; doSomething();` |
| 常に `true` または常に `false` になる条件式内の分岐 | `if (list != null && list != null)` |
| オーバーライドされたメソッド内の趋りから到達できないコード | 期待外の private メソッド |

**到達不能コードが検出された場合の対応**:

1. 該当コードの詳細（ファイル・メソッド・行）を記録する
2. **そのコードが API 実装に反映されているかどうか** を実装ファイルで確認する
3. 反映されている場合、以下のフォーマットで **必ずユーザーに通知する**:
...
API 実装に反映されているため、sonar-review エージェントがカバレッジ 100%
...
4. 通知後はユーザーの指示を待ち、指示された対応方針を Serena Memory（`behavior_verification_result`）に追記する
...
### 到達不能コード検出
- [ ] JSF Backing Bean の到達不能コードパターンを確認した（3-5）
- [ ] 到達不能コードが API 実装に反映されていた場合、所定フォーマットでユーザーに通知し、対応方針の指示を受けた
- [ ] ユーザー指示を `behavior_verification_result` Memory に追記した
...
- 到達不能コード検出結果とユーザー指示内容（検出なしの場合もその旨を記録）
- 最終テスト結果サマリー
```
...
- [ ] JSF Backing Bean の到達不能コードをステップ 3-5 の手順で確認した
