...
### 3. 自己改善ループ
- ユーザーから修正を受けたら必ず `.github\tasks\lessons.md` にそのパターンを記録する
...
1. **まず計画を立てる**：チェック可能な項目として `.github\tasks\todo.md` に計画を書く
2. **計画を確認する**：実装を開始する前に確認する
3. **進捗を記録する**：完了した項目を随時マークしていく
4. **変更を説明する**：各ステップで高レベルのサマリーを提供する
5. **結果をドキュメント化する**：`.github\tasks\todo.md` にレビューセクションを追加する
6. **学びを記録する**：修正を受けた後に `.github\tasks\lessons.md` を更新する

---

## コーディング規約

- すべてのファイルはBOMなしの `UTF-8` でエンコードすること
- いかなるファイルの先頭にも `BOM（U+FEFF）` を挿入してはならない
